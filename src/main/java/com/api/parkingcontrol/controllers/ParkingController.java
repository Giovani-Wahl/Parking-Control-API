...
public class ParkingController {
    ...
    @GetMapping("/report")
    public ResponseEntity<Page<ReportDTO>> getReport(@RequestParam(defaultValue = "") String licensePlateCar,
            @PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.ASC) Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK).body(service.searchByLicensePlateCar(licensePlateCar, pageable));
    }
